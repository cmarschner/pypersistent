...
1. Create PersistentDict pm1 from dict d
2. Do dict merge and sorted: sorted((d | d2).items())
3. Create PersistentDict pm2 from dict d2
...
where the dict merge affects objects stored in the PersistentDicts.

Workarounds:
- Use list(pm.items()) instead of pm.items() directly in sorted()
- Don't mix dict operations with the same dicts used to create PersistentDicts
