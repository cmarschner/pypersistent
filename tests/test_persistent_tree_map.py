"""
Tests for PersistentSortedDict - Immutable sorted map using red-black tree
...
class TestPersistentSortedDictBasics:
    """Test basic operations on PersistentSortedDict"""
    ...
    def test_dissoc_nonexistent(self):
        ...
        m = PersistentSortedDict.create(a=1, b=2)
        m2 = m.dissoc("z")
        assert m2 == m
    ...
    def test_get_missing_raises(self):
        ...
        m = PersistentSortedDict.create(a=1)
        with pytest.raises(KeyError):
            m.get("z")
...
class TestPersistentSortedDictOrdering:
    """Test that PersistentSortedDict maintains sorted order"""
...
class TestPersistentSortedDictRangeQueries:
...
class TestPersistentSortedDictImmutability:
    """Test that PersistentSortedDict is truly immutable"""

    def test_assoc_immutability(self):
        """Test that assoc doesn't modify original"""
        m1 = PersistentSortedDict.create(a=1, b=2)
        m2 = m1.assoc("c", 3)
    ...
    def test_dissoc_immutability(self):
        ...
        m1 = PersistentSortedDict.create(a=1, b=2, c=3)
        m2 = m1.dissoc("b")
    ...
    def test_multiple_versions(self):
        ...
        m1 = PersistentSortedDict.create(a=1)
        m2 = m1.assoc("b", 2)
        m3 = m2.assoc("c", 3)
        m4 = m1.assoc("d", 4)
...
class TestPersistentSortedDictFactoryMethods:
    """Test factory methods for creating maps"""

    def test_from_dict(self):
        """Test creating from Python dict"""
        d = {"a": 1, "b": 2, "c": 3}
        m = PersistentSortedDict.from_dict(d)
    ...
    def test_create(self):
        ...
        m = PersistentSortedDict.create(x=10, y=20, z=30)
...
class TestPersistentSortedDictPythonProtocols:
...
class TestPersistentSortedDictEdgeCases:
    """Test edge cases and special scenarios"""

    def test_none_as_value(self):
        """Test that None can be a value"""
        m = PersistentSortedDict.create(a=None, b=1)
        assert len(m) == 2
    ...
    def test_repr(self):
        ...
        m = PersistentSortedDict.create(a=1, b=2, c=3)
        r = repr(m)
        assert "PersistentSortedDict" in r
...
class TestPersistentSortedDictPerformance:
...
class TestPersistentSortedDictMergeOperator:
    ...
    def test_merge_two_treemaps(self):
        ...
        tm1 = PersistentSortedDict().assoc(1, 'a').assoc(2, 'b')
        tm2 = PersistentSortedDict().assoc(3, 'c').assoc(4, 'd')
        ...
        assert isinstance(result, PersistentSortedDict)
    
    def test_merge_with_overlap(self):
        """Test that right side wins on key conflicts."""
        tm1 = PersistentSortedDict().assoc(1, 'original').assoc(2, 'b')
        tm2 = PersistentSortedDict().assoc(2, 'UPDATED').assoc(3, 'c')
    ...
    def test_merge_with_dict(self):
        ...
        tm = PersistentSortedDict().assoc('a', 1).assoc('b', 2)
        d = {'c': 3, 'd': 4}
    ...
    def test_merge_with_persistent_map(self):
        """Test merging TreeMap with PersistentDict (hash-based)."""
        from pypersistent import PersistentDict

        tm = PersistentSortedDict().assoc(1, 'a').assoc(2, 'b')
        pm = PersistentDict.from_dict({3: 'c', 4: 'd'})
    ...
    def test_merge_preserves_sorted_order(self):
        ...
        tm1 = PersistentSortedDict().assoc(5, 'e').assoc(1, 'a')
        tm2 = PersistentSortedDict().assoc(3, 'c').assoc(7, 'g')
    ...
    def test_merge_immutability(self):
        ...
        tm1 = PersistentSortedDict().assoc(1, 'a').assoc(2, 'b')
        tm2 = PersistentSortedDict().assoc(3, 'c')
    ...
    def test_merge_large_maps(self):
        ...
        tm1 = PersistentSortedDict()
        for i in range(100):
            tm1 = tm1.assoc(i, f'val{i}')
        
        tm2 = PersistentSortedDict()
